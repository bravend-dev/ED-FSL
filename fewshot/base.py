...
class FSLBaseModel(nn.Module):
    ...
    def generate_interpolation_weights(self, shape, N, eps=None):
        """
        params:
            shape: (tuple) shape of the generated weights
            u: parameter of the Uniform distribution U(0,u)
        """
        ...
        if eps < 0 or eps > 1 / N:
            eps = 1 / N
        # Uniform U(0, u)
        u = math.log(eps * (N - 1) + 1) - math.log(1 - eps)

        weights = u * torch.rand(size=shape)
