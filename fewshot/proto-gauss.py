...
def gauss_distribution(support):
    """
    ...
    mean = support.mean(dim=2)  # B,N,K,D -> B,N,D
    cov_matrix = []
    for bi in range(B):
        for ni in range(N):
            cov = torch.cov(support[bi, ni, :, :].T)  # D x D
            cov_matrix.append(cov)
    cov_matrix = torch.stack(cov_matrix).view(B, N, D, D).contiguous()
...
def test_gauss():
    B, N, K, D = 2, 3, 5, 7
    support = torch.ones((B, N, K, D)).cuda()

    mu, sigma = gauss_distribution(support)
