...
class MELRTCFSLTrainer(torch.nn.Module):

    def __init__(self, fsl_model, train_dl, dev_dl, test_dl, args):
        super(MELRTCFSLTrainer, self).__init__()
    ...
    def do_train(self):
        ...
        for i, batch in enumerate(pbar):
            ...
            logits_fsl = return_item['logit'].view(-1, N)
            ...
            loss_fsl = self.ce(logits_fsl, target)
            cerc_loss_fsl = return_item['cerc_loss']

            batch2 = batch
            batch2['dep'] = batch['prune_dep']
            return_item2 = self.fsl_model(batch2, self.train_setting, training=True)
            logits_tc = return_item2['logit'].view(-1, N)

            loss_tc = self.ce(logits_tc, target)
            cerc_loss_tc = return_item2['cerc_loss']

            cerc_loss = (cerc_loss_fsl+cerc_loss_tc)

            total_loss = loss_fsl + self.args.alpha * cerc_loss + loss_tc
            ...
            if i % train_log_iter == 0 and i > 0:
                l = loss_fsl.detach().cpu().numpy()
                if not isinstance(cerc_loss, float):
                    c = cerc_loss.detach().cpu().numpy()
                else:
                    c = cerc_loss
                t = total_loss.detach().cpu().numpy()
                tc = loss_tc.detach().cpu().numpy()

                print(f'| @ {i} FSL={l:.4f} CERC={c:.4f} TC={tc:.4f} Total:={t:4f}')
