...
def schedule_tree_consistency(model=PROTO,
                              opt=DF_OPT,
                              encoder=BERTGCN,
                              layer=4,
                              dataset=DF_DATASET,
    ...
    for lr in [0.00001, 0.00002, 0.00003, 0.00004, 0.00005]:
        for seed in seeds:
            log = 'logs-tc/{dataset}/{encoder}/{N}-{K}.{model}.{encoder}.l-{layer}.{opt}-{lr}.s-{seed}.txt'.format(
                dataset=dataset,
                encoder=encoder,
                N=N,
                K=K,
            ...
            err = '{N}-{K}.{model}.l-{layer}.{opt}-{lr}.err'.format(
                dataset=dataset,
                encoder=encoder,
                N=N,
                K=K,
            ...
            command = 'python fsl-tree-consistency.py -n {N} -k {K} -m {model} -d {dataset} --lr {lr} -e {encoder} --seed {seed}'.format(
                dataset=dataset,
                encoder=encoder,
                model=model,
                N=N,
...
if __name__ == '__main__':
    ...
    # schedule_tree_consistency(model='proto', dataset='rams')
    schedule_tree_consistency(model='melr', dataset='ace')
    schedule_tree_consistency(model='melr', dataset='rams')
